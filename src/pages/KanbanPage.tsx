...
export default function KanbanPage() {
  ...
  <Modal open={!!selected} title='카드 상세' onClose={()=>setSelected(null)}>{selected && <div><p>{selected.title}</p><p>{selected.content}</p><div className='inline'><button onClick={async()=>{await kanbanApi.deleteCard(selected.cardId);setSelected(null);reload();}}>삭제</button></div></div>}</Modal>
  </section>;
}
