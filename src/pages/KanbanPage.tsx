...
export default function KanbanPage() {
  ...
  return <section className='panel'><h2>Kanban (Trello 스타일)</h2><div className='inline'><input placeholder='컬럼 이름' value={newColumn} onChange={e=>setNewColumn(e.target.value)}/><button onClick={async()=>{if(!board||!newColumn)return; await kanbanApi.createColumn(board.boardId,{name:newColumn,order:columns.length+1}); setNewColumn(''); reload();}}>+ 컬럼 추가</button></div><div className='kanban-wrap'>{columns.map(col => <div key={col.columnId} className='k-col' draggable onDragStart={() => setDragCol(col.columnId)} onDragOver={(e)=>e.preventDefault()} onDrop={() => onColumnDrop(col)}><div className='k-head'><b>{col.name}</b><button onClick={()=>{setTargetCol(col.columnId);setOpen(true);}}>+ 카드 추가</button></div><div className='k-cards' onDragOver={(e)=>e.preventDefault()} onDrop={()=>onCardDrop(col.columnId)}>{(col.cards ?? []).map(card => <article key={card.cardId} className='k-card' draggable onDragStart={() => setDragCard(card)} onClick={()=>setSelected(card)}><h4>{card.title}</h4><p>{card.content || '내용 없음'}</p><small>작성자: {card.createdByUserId}</small><small>마감: {card.dueDate?.slice(0,10) ?? '-'}</small><small>업데이트: {card.updatedAt?.slice(0,16).replace('T',' ') ?? '-'}</small></article>)}</div></div>)}</div>
  ...
  <Modal open={!!selected} title='카드 상세' onClose={()=>setSelected(null)}>{selected && <div><div className='inline'><input value={selected.title} onChange={e=>setSelected({...selected,title:e.target.value})}/><input value={selected.content||''} onChange={e=>setSelected({...selected,content:e.target.value})}/><input type='date' value={(selected.dueDate||'').slice(0,10)} onChange={e=>setSelected({...selected,dueDate:e.target.value})}/></div><div className='inline'><button onClick={async()=>{await kanbanApi.updateCard(selected.cardId,{title:selected.title,content:selected.content,dueDate:selected.dueDate});reload();}}>수정 저장</button><button onClick={async()=>{await kanbanApi.deleteCard(selected.cardId);setSelected(null);reload();}}>삭제</button></div></div>}</Modal>
  </section>;
}
