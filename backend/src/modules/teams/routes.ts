...
export const teamsRoutes: FastifyPluginAsync = async (app) => {
  ...
  app.post('/teams', { schema: { tags: ['Teams'], body: { type:'object', required:['name'], properties:{ name:{type:'string'} } } }, preHandler:[app.requireAuth] }, async (req:any)=> app.prisma.team.create({data:{name:req.body.name,joinCode:crypto.randomUUID()}}));
